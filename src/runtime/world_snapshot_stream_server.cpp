...
WorldSnapshotStreamServer::WorldSnapshotStreamServer(WorldSnapshotStreamServerConfig config)
    : config_(std::move(config)) {}

WorldSnapshotStreamServer::~WorldSnapshotStreamServer() {
    stop();
}

void WorldSnapshotStreamServer::start() {
    ...
    if (::inet_pton(AF_INET, config_.bind_host.c_str(), &addr.sin_addr) != 1) {
        close_listen_socket();
        running_ = false;
        throw std::runtime_error("invalid world snapshot stream bind host: " + config_.bind_host);
    }
    ...
    if (::bind(listen_fd_, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        ...
        close_listen_socket();
        running_ = false;
        throw std::runtime_error("bind() failed for world snapshot stream: " + message);
    }
    ...
    if (::listen(listen_fd_, config_.listen_backlog) < 0) {
        ...
        close_listen_socket();
        running_ = false;
        throw std::runtime_error("listen() failed for world snapshot stream: " + message);
    }

    accept_thread_ = std::thread([this]() { accept_loop(); });
}

void WorldSnapshotStreamServer::stop() {
...
void WorldSnapshotStreamServer::on_snapshot(const WorldSnapshot& snapshot) {
    ...
    const auto line = stream_line_for(seq, snapshot);
...
std::uint16_t WorldSnapshotStreamServer::port() const {
    return config_.port;
}

WorldSnapshotStreamServerStats WorldSnapshotStreamServer::stats() const {
    std::lock_guard<std::mutex> lock{mutex_};
    return WorldSnapshotStreamServerStats{
...
void WorldSnapshotStreamServer::accept_loop() {
    while (running_) {
...
void WorldSnapshotStreamServer::close_listen_socket() {
    const int fd = listen_fd_;
    listen_fd_ = -1;
    close_fd(fd);
}

void WorldSnapshotStreamServer::close_all_clients() {
    std::lock_guard<std::mutex> lock{mutex_};
