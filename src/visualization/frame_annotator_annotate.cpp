...
// Each glyph = 5 bytes; each byte = one column, bit 0 = top row, bit 6 = bottom row.
// Source: Adafruit GFX library font (public domain).
static const std::uint8_t kFont5x7[95][5] = {
...
void draw_char(ImageView& image, const int x, const int y,
               const char ch, const RgbColor fg) {
    const auto code = static_cast<unsigned char>(ch);
    if (code < 32U || code > 126U) {
        return;
    }
    const auto* glyph = kFont5x7[code - 32U];
    for (int col = 0; col < 5; ++col) {
        const std::uint8_t bits = glyph[col];
        for (int row = 0; row < 7; ++row) {
            if (bits & static_cast<std::uint8_t>(1U << row)) {
                set_pixel(image, x + col, y + row, fg);
            }
        }
    }
}
...
void draw_label_bar(ImageView& image, int x, int y, const std::string& text, const RgbColor fg, const RgbColor bg) {
    ...
    const int pad    = 1;
    const int bg_w = std::min(
        static_cast<int>(text.size()) * char_w + pad * 2,
        std::max(1, image.width - safe_x));
    const int bg_h = 7 + pad * 2;  // 7px glyph height + 1px padding top/bottom
    fill_rect(image, safe_x, safe_y, bg_w, bg_h, bg);
    for (std::size_t i = 0; i < text.size(); ++i) {
        const int cx = safe_x + pad + static_cast<int>(i) * char_w;
        if (cx + 5 > image.width) {
            break;
        }
        draw_char(image, cx, safe_y + pad, text[i], fg);
    }
}
