...
struct Args {
    ...
    std::filesystem::path output_dir{"out/replay_mission_snapshots"};
    int max_frames{0};
...
private:
    ...
    void emit(int frame_count, bool done) const {
        std::cerr << "\r\033[Kdedalus_replay_mission: processed " << frame_count;
        if (max_frames_ > 0) {
...
int main(int argc, char** argv) {
    try {
        ...
        if (frame_count == 0) {
            std::cerr << "dedalus_replay_mission: no frames processed\n";
            return 1;
        }
        ...
        std::cerr << "dedalus_replay_mission: " << ex.what() << "\n";
        return 2;
    }
}
