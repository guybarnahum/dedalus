...
**19. No integration test exercises `MissionRuntime` with a real `FlightCommandSink`**  ✅ **FIXED**  
...
*Resolution:* `tests/fixtures/px4_bridge_ci_stub.py` — a lightweight Python stub that implements the full `px4-command-bridge.py` JSON pipe protocol: it emits `{"ok":true,"status":"ready"}` on startup, then responds to `arm`, `takeoff`, `velocity`, `land`, `disarm`, and `shutdown` commands with `{"ok":true,...}` responses, echoing the received `vx`/`vy`/`vz` fields in velocity replies. Output uses compact separators so the C++ `json_bool_value()` parser finds `"ok":true` without whitespace. The `px4_bridge_command_sink` contract test forks the stub via the real `Px4BridgeCommandSink` constructor, drives all five command kinds, verifies `success == true` and response content for each, confirms velocity clamping is reflected in the stub echo (`"vx":2` from an input of `3.0` with `max_velocity_mps = 2.0`), and verifies that a bridge executable which exits without printing a ready signal causes the constructor to throw `std::runtime_error`. 35/35 tests pass.

**20. `dedalus_ghost_scenario_eval` binary has no CI coverage**  ✅ **FIXED**  
...
*Resolution:* `tests/integration/test_ghost_scenario_eval_cli.py` — a synthetic-layer CTest entry that runs `dedalus_ghost_scenario_eval --scenario config/behaviors/ghost_detections/person_pair_crossing.json --time-s 10`, parses the JSON output, and asserts schema version, scenario name, map frame ID, and the positions/velocities/sizes of all three detections (`ghost_person_001`, `ghost_person_002`, `ghost_car_001`) to full floating-point precision. The binary is covered at the scenario layer; ghost correctness is no longer inferred only through mission smoke tests. 35/35 tests pass.
