...
**6. `CoreStackRunner` wires subscribers manually in the application binary**  
...
*Suggested fix:* Either implement it or remove the class and its config path, and re-add it when the implementation exists.
...
**16. TCP clients that fall behind are silently disconnected; no backpressure signaling**  
...
**17. `ArtifactSnapshotWriter` queue depth is hardcoded at 64**  
